...
(function(){
  ...
  async function loadArticles(target){
    ...
    const out=[];
    for(let i=0;i<items.length&&out.length<target;i+=4){const batch=await Promise.all(items.slice(i,i+4).map(readArticle));out.push(...batch.filter(Boolean));}
