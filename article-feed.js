...
(function(){
  ...
  const card=(a,index=99)=>{const priority=index===0?'high':'low';const loading=index===0?'eager':'lazy';return `<article class="article-card" data-search="${esc(a.title+" "+a.description+" "+a.category)}"><img src="${esc(a.image||FALLBACK)}" alt="${esc(a.title)}" loading="${loading}" decoding="async" width="1200" height="675" fetchpriority="${priority}" data-feed-fallback="${FALLBACK}"><div class="article-body"><span class="tag">${esc(a.category)}</span><h3>${esc(a.title)}</h3><p>${esc(a.description)}</p><a href="${esc(a.url)}">Read article →</a></div></article>`};
  ...
  function deferMonetag(){const run=()=>{try{loadMonetag()}catch(e){}};if("requestIdleCallback" in window)requestIdleCallback(run,{timeout:5000});else setTimeout(run,3000)}
