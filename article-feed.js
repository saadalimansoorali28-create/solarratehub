/* Automatic latest-article feed. Keeps the homepage light and uses the article's real content image, not the site logo. */
(function(){
  ...
  function firstImage(doc,base){
    const candidates=[...doc.querySelectorAll("main img,article img,.article-page img,.wrap img")];
    const img=candidates.find(el=>{const src=el.getAttribute("src")||"";return src&&!el.closest("header,nav")&&!/95129107-17c4-4326-83cd-09252251a676\.png/i.test(src)});
    if(!img)return "";
    try{return new URL(img.getAttribute("src"),base).href}catch(e){return img.getAttribute("src")||""}
  }
  ...
  async function loadArticles(limit){
    ...
    let items=[...doc.querySelectorAll("url")].map(u=>({loc:u.querySelector("loc")?.textContent||"",lastmod:u.querySelector("lastmod")?.textContent||""})).filter(x=>x.loc&&!EXCLUDED.has(clean(x.loc)));
    items.sort((a,b)=>(Date.parse(b.lastmod||"")||0)-(Date.parse(a.lastmod||"")||0));
    if(limit)items=items.slice(0,limit);
    const out=[];
    for(let i=0;i<items.length;i+=4){const batch=await Promise.all(items.slice(i,i+4).map(readArticle));out.push(...batch.filter(Boolean));}
